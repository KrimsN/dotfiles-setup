...
rcfile::remove_block() {
  ...
  local tmp
  tmp="$(mktemp)"
  # Вместе с блоком снимаем и пустую строку прямо перед ним — ту самую,
  # которую дописал upsert_block (см. `echo ""` выше). Без этого файл
  # после удаления блока не возвращается к исходному виду, а копит по
  # пустой строке за каждый снятый блок; на `knrc uninstall`, который
  # снимает из ~/.bashrc сразу три блока, это уже заметно.
  # Придерживаем ровно одну пустую строку: подряд идущие пустые строки
  # пользователя при этом сохраняются, кроме последней перед маркером.
  awk -v begin="$begin" -v end="$end" '
