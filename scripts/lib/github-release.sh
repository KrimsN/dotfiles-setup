...
# "/usr/bin/tool" или хотя бы "/tool" (с ведущим "/") — без него
# "*<glob>" совпадёт по подстроке с чем угодно, оканчивающимся на
# <glob>, например "completions/_zoxide" тоже совпадёт с глобом
# "zoxide" (баг, воспроизведённый на практике: вместо бинарника
# ставился zsh-completion скрипт zoxide). Если ассет — не .tar.gz, а
# голый бинарник (например jq публикует именно так), inner_path_glob
