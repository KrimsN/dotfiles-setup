...
## Откат (`knrc uninstall`)

Бинарник ставится распаковкой релизного архива поверх `/usr/local`
(`bin/` + `lib/` + `share/`), и точного списка распакованных файлов
модуль не сохраняет. Поэтому откат
([uninstall.md](uninstall.md)) удаляет три пути, которые заведомо
принадлежат только neovim — `/usr/local/bin/nvim`,
`/usr/local/lib/nvim`, `/usr/local/share/nvim`, — а про возможные
остатки (man-страница, `.desktop`, иконки) печатает строку в отчёт
«осталось» с готовой командой `ls`. Если `nvim` нашёлся не в
`/usr/local/bin`, он поставлен не нами и не трогается вовсе.

Всё, что nvim насоздавал вокруг себя (плагины lazy.nvim, парсеры
treesitter, `lazy-lock.json`, luac-кэш `~/.cache/nvim`), существует
только благодаря нашему `init.lua` — откат удаляет их, **только если
`init.lua` всё ещё наш**. `~/.local/state/nvim` не удаляется никогда:
там shada с метками, регистрами и историей пользователя.

Если модуль когда-нибудь начнёт класть файлы куда-то ещё — добавить их в
`uninstall::plan_nvim` и прогнать
`scripts/test-uninstall.sh ubuntu24 "base,nvim"`.
